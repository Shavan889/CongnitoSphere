      ...
      {messages.map((m,index) => (
        <div key={index} className={`msg msg-${m.type}`}>
          <div className="msg-header">
            <span className="msg-role" aria-hidden="true">{m.type === 'user' ? 'You' : 'AI'}</span>
            {m.timestamp && (
              <>
                <span className="msg-header-separator">•</span>
                <span className="msg-time" title={formatFullDate(m.timestamp)}>
